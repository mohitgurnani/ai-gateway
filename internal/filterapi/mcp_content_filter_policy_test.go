...
// After the gateway slimming refactor, MCPContentFilterPolicy carries
// only the GlobalDisable kill switch. All in-process knobs (PII, cache,
// breaker, Jira, backends, eval header, wire limits) live with the
// dispatcher service in panacea-agent and are tested there.

// TestPolicy_GlobalDisableDefaultsFalse asserts that a freshly
...
// filter.
func TestPolicy_GlobalDisableDefaultsFalse(t *testing.T) {
	var p MCPContentFilterPolicy
	require.False(t, p.GlobalDisable, "zero-value policy must have GlobalDisable=false")
}

// TestPolicy_GlobalDisableExplicit asserts the field is round-trippable
// when set explicitly. The struct is intentionally tiny so this is mostly
// a regression guard against an accidental field rename or omitempty
// regression that would silently flip the kill switch.
func TestPolicy_GlobalDisableExplicit(t *testing.T) {
	p := MCPContentFilterPolicy{GlobalDisable: true}
	require.True(t, p.GlobalDisable)
}

// TestPolicy_JSONRoundTrip asserts the policy serializes to the wire
