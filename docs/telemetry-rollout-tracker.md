...
| Last updated | 2026-04-27 (P3f Phase A `auth.kind` / `client.address` / IP-fallback `user.id` stacked on PR #10) |
|---|---|
...
| Active gateway image | `local/ai-gateway-cli:cf-stack` rebuilt 2026-04-27 (late) from `mohit/lf-input-output-capture` (commit `f796f6c9`, stacked on `ad29a7a7`) — both `aigw-cf-stack-gateway` and `aigw-cf-stack-eval-gateway` have been recreated against this image (post-P3e redeploy) and report `(healthy)` |
